---
title: '按项目规模选型'
description: 'Java 后端按项目规模选型文档，梳理小型单体、中型微服务和大型企业系统的技术组合建议。'
---
...
> 版本基线同样不随规模变化：Java 基础原理和存量项目维护以 JDK 8 为主；现代 Spring Boot 3.x 新项目最低使用 Java 17，在整套依赖兼容时选择 Java 17 / 21，优先评估 Java 21 LTS。
...
| 缓存 | `Redis + Redisson` |
| 搜索 | `Elasticsearch` |
| 消息队列 | `RocketMQ / Kafka / Pulsar`，按业务消息、流式数据、多租户平台需求选择 |
| 文件存储 | `MinIO / OSS` |
| 定时任务 | `XXL-Job` |
...
Spring Boot 3.x
Java 17 / 21
与 Spring Boot 小版本匹配的 Spring Cloud Release Train
...
其中 MQ 根据场景在 RocketMQ、Kafka、Pulsar 中选择，不要求同时引入；RabbitMQ 可作为传统 AMQP 方案参考，但本站不单独维护其技术页。
