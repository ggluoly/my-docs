...
面向对象把系统拆成对象，通过对象的属性和行为表达业务；面向过程更关注步骤和函数调用。
面向对象更强调封装、继承、多态和职责划分，适合复杂业务的扩展和维护；面向过程实现直接，适合流程简单、性能敏感或脚本化的场景。

#### 常见追问

- 面向对象一定比面向过程好吗？
  不一定，它们是解决不同问题的范式，没有绝对优劣。
  **面向对象**适合业务复杂、领域模型稳定、需要封装状态和扩展行为的场景，通过封装、继承、多态提升可维护性。
  **面向过程**更适合流程明确、数据转换或性能敏感的场景，执行路径直观，开发成本低。
- 封装解决了什么问题？
  主要解决数据和行为散落、对象内部状态可被随意修改的问题。

### 面向对象的三大特性是什么？

面向对象三大特性是**封装、继承和多态**。
**封装**隐藏对象内部细节，只暴露必要接口；**继承**复用父类能力并表达父子关系；**多态**让同一个接口在不同实现类上表现出不同行为。
工程中要避免为了继承而继承，优先用清晰职责和组合降低耦合。

#### 常见追问

- 多态的前提是什么？
  要有继承或接口实现关系，子类继承父类，或者实现同一个接口。
  子类需要重写父类的实例方法，方法签名要保持一致。
  父类或接口的引用指向子类对象。
- 继承和组合怎么选？
  优先选择组合，继承只在明确满足子类确实是父类的一种关系时使用。
- 封装是不是只靠 private 实现？
  不是。private 只是实现封装最常用的手段之一，用来隐藏字段或内部方法，防止外部直接修改对象状态。
  封装的核心是：隐藏实现细节，只暴露必要且受约束的操作，保证对象状态和业务规则的一致性。
  还可以通过包访问权限、接口隔离、不可变对象、只读集合、DTO 与领域对象分离等方式实现封装。
...
- [基础语法](/java-core/basic-syntax)

### main 方法是什么？

`main` 是传统 Java 应用的启动入口。使用 `java 类名` 启动程序时，Java 启动器会加载指定类并调用其 `main` 方法。最常见的声明是：
...
- `public`：让 Java 启动器能够访问该方法。
- `static`：无需先创建对象，类加载后即可调用。
- `void`：启动器不使用该方法的返回值。
- `String[] args`：接收启动命令传入的命令行参数。

`String... args` 与 `String[] args` 等价，也可以作为传统入口参数。Java 21+ 支持更灵活的 main 方法形式，但常规工程和面试中仍应优先掌握 `public static void main(String[] args)`。

#### 常见追问

- main 方法里面的 `String[] args` 参数是干什么的？
  `String[] args` 用于接收启动 Java 程序时传入的命令行参数。参数会按命令行规则拆分后依次放入数组，每一项都是字符串。
  例如执行 `java Demo dev 8080` 时，`args[0]` 是 `"dev"`，`args[1]` 是 `"8080"`。业务代码可再将它们解析为环境、端口、文件路径等配置。
  它和 JVM 参数不同：`-Xmx1g`、`-Dspring.profiles.active=dev` 等 JVM 参数应写在类名前；类名后的内容才会传入 `args`。使用 `java -jar app.jar --server.port=8080` 时，`--server.port=8080` 也会进入应用入口参数，具体框架再决定如何解析。
...
- [Java 8+ 特性](/java-core/features/java-8)
...
- `String` 的 `+` 是不是运算符重载？
  Java 不支持开发者自定义运算符重载，+ 对 String 的拼接是 Java 语言规范内置支持的特殊规则。
  编译期常量拼接，例如 `"a" + "b"`，可能直接折叠为一个字符串常量。对于非常量拼接，JDK 8 的 `javac` 通常生成 `StringBuilder.append()` 调用；JDK 9+ 默认通常使用 `invokedynamic` 和 `StringConcatFactory`。具体字节码实现会随 JDK 变化，业务代码不应依赖它。
- Java 为什么保留方法重载？
  为了提升 API 的易用性和表达能力。
  同一个业务动作可能接受不同类型、不同数量的参数；还能支持默认值效果；编译器会根据参数个数、类型和顺序确定调用哪个方法
- 运算符重载有什么风险？
  最大的风险是语义不直观。调用方很难仅从代码判断实际行为和复杂度。
...
- 接口多继承会不会产生冲突？
  会，但要分情况。
  **抽象方法同签名** 两个接口有相同的抽象方法签名时，通常不冲突，实现类实现一次即可。
  **默认方法同签名** 两个接口都提供了相同签名的 default 方法，实现类会产生冲突，必须显式重写
  **父接口与子接口的默认方法** 如果一个接口继承另一个接口，子接口的默认方法优先，遵循“更具体的接口优先”，一般不会冲突。
  **类与接口默认方法冲突** 如果父类中已有同签名方法，父类方法优先，接口的默认方法不会覆盖它。
- 默认方法冲突怎么解决？
  实现类必须显式重写冲突的默认方法，由实现类决定最终逻辑。
- 组合为什么通常比继承更灵活？
  它把“复用能力”和“类型继承”分开了。
...
- 默认访问和 `protected` 有什么区别？
  **默认访问权限**：不写修饰符，只能被同一个包中的类访问；不同包即使是子类也不能访问。
  **protected**：同包内任何类都能访问；不同包时，访问代码必须位于子类中，并且通常只能通过 `this`、`super` 或编译期类型为该子类及其子类的引用访问，不能通过任意父类类型对象访问。
- 顶级类可以用哪些访问修饰符？
  顶级类只能使用 public 和默认访问权限（不写修饰符），顶级类没有外部类作为访问边界。
- 为什么字段通常不直接设置为 public？
  主要是为了封装。外部可以直接读写字段，绕过校验逻辑，容易破坏对象状态；无法控制写入时机和副作用；后续难以演进；不利于线程安全，直接暴露可变字段很难统一控制并发访问。

### Java 中常见非访问修饰符有哪些？

常见非访问修饰符包括 `static`、`final`、`abstract`、`synchronized`、`volatile` 和 `transient`。
`static` 表示类级成员；`final` 表示不可变引用、不可重写方法或不可继承类；`abstract` 用于抽象类和抽象方法；`synchronized` 用于同步；`volatile` 保证可见性和有序性；`transient` 表示序列化时忽略字段。

#### 常见追问

- `final` 修饰引用类型表示对象不可变吗？
  final 修饰引用类型，表示引用本身不能再指向其他对象，但对象内部状态是否能修改，取决于对象本身。
- `volatile` 和 `synchronized` 有什么区别？
  volatile 保证变量的可见性，并禁止相关的指令重排序；但不保证复合操作的原子性，例如 count++。不会阻塞线程，适合状态标记、开关控制等场景。
  synchronized 通过加锁实现互斥，同一时间只有一个线程能进入临界区，同时保证原子性、可见性和有序性。适合共享资源修改、计数器递增、多变量一致性等需要临界区保护的场景。
- `transient` 常用于什么场景？
  用于让某个成员变量不参与 Java 原生序列化。对象反序列化后，transient 修饰的引用类型字段通常是 null，基本类型则是默认值，例如 int 为 0。
...
- `import java.util.*` 会不会影响运行性能？
  不会影响运行性能。
  import java.util.* 只是编译阶段告诉编译器：使用 java.util 包下的类时，可以省略包名。编译后，字节码里引用的都是具体类，不会在运行时去扫描整个包。
- 静态导入适合哪些场景？
  适合“方法名或常量含义很明确”的场景，可以省掉类名前缀。
  在业务代码里会谨慎使用，尤其不建议大量 import static xxx.*。因为看到 save()、of()、equals() 这类方法时，不容易判断它属于哪个类，也容易和其他静态方法重名。
  一般来说，测试代码可以多用；业务代码只对少量、语义明确的常量或工具方法使用。
- 为什么不建议大量使用通配符导入？
  不是性能问题，而是代码可读性和维护性问题。
  看不出来具体依赖了哪些类。容易出现类名冲突。后续维护风险更高。对代码审查和依赖分析不友好。
...
- 字符串比较为什么要用 `equals()`？
  `==` 比较两个引用是否代表同一对象；`String.equals()` 比较字符序列内容是否相同。Java 不暴露对象真实内存地址，因此不应把引用比较理解为比较内存地址。

#### 关联文档

- [字符串](/java-core/basic-syntax)
...
- `==` 和 `equals()` 有什么区别？
  `==` 对于基本数据类型比较值；对于引用类型比较两个引用是否代表同一对象。
  `equals()` 是 `Object` 中的方法，默认实现与 `==` 一样比较对象身份；很多类会重写它，以定义逻辑上的相等规则，例如 `String` 比较字符序列内容。
- 为什么哈希冲突不可避免？
...
- `char[]` 一定安全吗？
  不一定安全，只能说比 String 更适合存放密码这类敏感信息。
  String 是不可变的，内容一旦创建就不能主动清除，只能等 GC，而且还可能因为字符串常量池、日志、异常、内存 dump 等原因暴露更久。
  char[] 的优势是可变，用完之后可以主动覆盖。
  **char[] 不是“一定安全”，只是给了“尽早清除敏感数据”的能力。**
- 为什么日志里不能打印密码？
  日志的生命周期和访问范围通常比业务数据更不可控。
  日志会落盘、被采集到 ELK/Splunk、被备份、被多人排查问题时查看，甚至可能长期保存。如果密码被打印进去，就相当于把敏感信息扩散到了很多系统里，很难彻底删除。另外日志经常用于排查线上问题，权限控制可能没有数据库那么严格，一旦账号泄露、日志平台被越权访问，明文密码就会直接暴露。
- 配置文件中的密码应该怎么保护？
  不建议明文写死，尤其不能提交到 Git。
  区分环境配置。开发、测试、生产的密码不要混在一起，生产密码只放在生产环境。
  通过环境变量、启动参数、K8s Secret、配置中心或者密钥管理服务来注入，而不是直接写在 application.yml 里。
  如果配置文件里必须放，也要加密存储，比如用 Jasypt 或公司统一的加密组件，应用启动时再解密，而且解密密钥不能和密文放在同一个文件里。
  控制权限。配置文件、CI/CD 变量、配置中心都要限制访问权限，避免普通开发或无关人员能看到生产密码。
  避免泄露到日志和仓库。.gitignore 要排除本地敏感配置，提交前可以配合 secret scan，日志里也不能打印完整配置。
  定期轮换密码。
...
- 如何减少空指针异常？
  明确对象是否允许为 null。如果参数不能为空，就在入口处校验，比如用 Objects.requireNonNull()，或者用 @NonNull、@NotNull 这类注解配合静态检查。
  避免返回 null。集合返回空集合，比如 Collections.emptyList()，字符串可以返回空串，业务上确实可能没有值时可以用 Optional 表达。
  调用前做好判空，尤其是外部接口、数据库查询、缓存读取、Map 取值这些地方，因为这些数据源都不完全可控。
  写代码时把常量放前面比较，比如 "SUCCESS".equals(status)。
  使用工具方法简化判空，比如 StringUtils.isBlank()、CollectionUtils.isEmpty()，减少手写判空遗漏。
  设计上尽量减少对象的中间状态，比如构造函数保证必要字段不为空，或者用 Builder 创建完整对象。
  测试覆盖一些空值场景，特别是边界条件和外部依赖返回空的情况。
- `Optional` 适合用在字段里吗？
  Optional 适合用在方法返回值，不推荐用在成员字段、方法参数、DTO 或实体类字段里。
- 自动拆箱为什么可能触发空指针？
  因为包装类型在转换成基本类型时，底层其实会调用对应的方法，比如 Integer 拆成 int 时会调用 integer.intValue()。如果这个包装对象本身是 null，那调用 intValue() 就会直接抛 NullPointerException。

### 方法重载和方法重写有什么区别？

重载是同一类型中可见的同名、不同参数列表的方法之间的关系，可以包含继承而来的方法，属于编译期绑定；重写发生在类继承、接口继承或类实现接口时，对可覆盖实例方法重新实现，属于运行期多态。重载不能只靠返回值区分，重写要求方法签名兼容，并且访问权限不能更严格。

#### 常见追问

- 重载能不能只改返回值？
  Java 方法重载只看方法名和参数列表，也就是参数个数、参数类型、参数顺序，不看返回值类型。
- 重写时异常声明有什么限制？
  重写时异常声明的限制主要看是不是受检异常。
  如果父类方法没有声明受检异常，子类重写时不能新增受检异常。
  如果父类方法声明了受检异常，子类重写时只能声明相同的异常，或者它的子类异常，不能声明更大范围的受检异常。
- `@Override` 有什么作用？
  告诉编译器：这个方法是要重写父类或者实现接口的方法。
...
- static 方法为什么没有多态？
  没有多态，是因为static属于类，不属于对象。
  多态的核心是运行期根据实际对象类型动态绑定方法，比如父类引用指向子类对象，调用实例方法时会走子类重写的方法。static 方法在编译期就已经根据引用类型确定了调用哪个类的方法，它是静态绑定，不参与运行期动态分派。
- final 方法能被重写吗？
  final 修饰的方法不能被子类重写，编译器会直接报错。
  final 方法可以被子类继承和调用，只是不能重写。
- private 方法能被重载吗？
  可以被重载，因为重载发生在同一个类里，只要方法名相同，参数列表不同就可以。
...
- [异常体系](/java-core/exceptions)

### try-catch-finally 哪些部分可以省略？

普通的 `try { ... }` 中，`catch` 和 `finally` 可以省略其中一个，但不能同时省略。Java 7+ 的 try-with-resources 例外：`try (resource) { ... }` 可以既没有 `catch`，也没有 `finally`，资源仍会自动关闭。
...
- [注解与反射](/java-core/annotations-reflection)
...
- Spring IoC 为什么大量使用反射？
  核心原因是 Spring 是一个通用容器，它在编译期并不知道交给哪些类，只能在运行时根据配置、注解和 BeanDefinition 去动态创建对象和装配依赖。反射的好处是灵活和解耦，会缓存 Constructor、Method、Field 这些元数据，避免每次重复解析。
...
- JDK 8 的 `HashMap` 为什么引入红黑树？
  解决哈希冲突严重时链表查询效率低的问题。
  JDK 8 中，当桶中已有 8 个节点、再插入第 9 个节点时会触发树化判断；数组容量至少为 64 时会树化，否则优先扩容。树化后，冲突严重时的查找复杂度可从 O(n) 优化到 O(log n)。
...
- [集合框架](/java-core/collections)
- [并发容器](/java-core/concurrent-containers)

### HashMap 的实现原理是什么？

JDK 8 中 `HashMap` 底层是数组 + 链表 + 红黑树。`put` 时先根据 key 的 hash 值定位桶位置，如果桶为空直接放入；如果发生哈希冲突，则在链表或红黑树中比较 key。当桶中已有 8 个节点、再插入第 9 个节点时会触发树化判断；数组容量至少为 64 时树化，否则优先扩容。默认初始容量是 16，负载因子是 0.75，扩容通常变为原来的 2 倍。
...
- 链表什么时候树化？
  JDK 8 中，桶中已有 8 个节点、再插入第 9 个节点时会触发树化判断；数组容量大于等于 64 时，链表转为红黑树，否则优先扩容。
- `HashMap` 扩容为什么可能影响性能？
...
数组排序常用 `java.util.Arrays`，以下方法都会直接修改原数组。

1. **基本类型数组升序：`Arrays.sort()`**
   ```java
   int[] numbers = {3, 1, 2};
   Arrays.sort(numbers); // [1, 2, 3]
   ```
   适合 `int[]`、`long[]`、`double[]` 等基本类型数组，默认按升序排列。基本类型数组没有接收 `Comparator` 的重载；若要降序，通常先升序排序再原地反转，或改用 `Integer[]` 等包装类型数组。

2. **对象数组或自定义规则：`Arrays.sort(array, comparator)`**
   ```java
   String[] names = {"Bob", "Alice", "Carol"};
   Arrays.sort(names, Comparator.reverseOrder());
   ```
   对象实现 `Comparable` 时可按自然顺序排序；更常见的是传入 `Comparator`，按对象字段、降序或多字段规则排序。对象数组排序是稳定的，相等元素会保持原有相对顺序。

3. **只排序部分元素：区间排序**
   ```java
   Arrays.sort(numbers, 1, 3);
   ```
   只排序 `[fromIndex, toIndex)` 区间，`fromIndex` 包含、`toIndex` 不包含，适合保留数组其他区间不变的场景。

4. **大数组并行排序：`Arrays.parallelSort()`**
   ```java
   Arrays.parallelSort(numbers);
   ```
   适合数据量足够大、排序是 CPU 密集型且公共 `ForkJoinPool` 没有被其他任务占满的场景。小数组的并行调度成本可能更高，默认优先使用 `Arrays.sort()`。

基本类型数组不应依赖相等元素的相对顺序。对象数组比较器还要满足自反性、传递性等约定，否则排序可能抛出异常或得到不可预期的结果。

#### 常见追问

- `Arrays.sort()` 和 `Arrays.parallelSort()` 怎么选？
  默认优先使用 `Arrays.sort()`，实现简单且没有并行调度开销。
  `Arrays.parallelSort()` 适合足够大的数组、CPU 核数充足且公共 `ForkJoinPool` 没有被其他任务占满的场景；它会并行排序并合并结果，不应因为“能并行”就默认使用。
- 基本类型数组如何降序排序？
  `int[]` 等基本类型数组没有接收 `Comparator` 的 `Arrays.sort()` 重载。常见做法是先用 `Arrays.sort()` 升序排序，再原地反转；如果需要按复杂规则排序，可改用 `Integer[]` 等包装类型数组并传入 `Comparator`，但会有装箱和额外内存开销。
- 排序对象数组要注意什么？
  对象实现了 `Comparable` 时可以按自然顺序排序；更常见的是传入 `Comparator`，例如 `Comparator.comparing(User::getAge).thenComparing(User::getId)`。比较器要满足自反性、传递性等约定，否则排序可能抛出异常或得到不可预期的结果。

### List 排序有哪些方式？

List 排序通常分为“直接修改原集合”和“生成新的排序结果”两类。

1. **Java 8+ 推荐：`list.sort(comparator)`**
   ```java
   List<String> names = new ArrayList<>(List.of("Bob", "Alice", "Carol"));
   names.sort(Comparator.naturalOrder());
   names.sort(Comparator.reverseOrder());
   ```
   这是最常用的原地排序方式。元素实现 `Comparable` 时可传 `null` 使用自然顺序；对象列表通常传入 `Comparator` 按字段排序。

2. **旧代码常见：`Collections.sort()`**
   ```java
   Collections.sort(names);
   Collections.sort(names, Comparator.reverseOrder());
   ```
   效果同样是原地排序，主要用于兼容 Java 8 之前或已有的工具类风格代码；新代码通常优先使用 `list.sort(comparator)`。

3. **不修改原集合：`stream().sorted()`**
   ```java
   List<User> sortedUsers = users.stream()
       .sorted(Comparator.comparing(User::getAge).thenComparing(User::getId))
       .toList();
   ```
   `sorted()` 返回新的 Stream，收集后得到新的 List，原集合保持不变。适合需要保留原始顺序、在多个排序视图之间切换的场景。

4. **按多个字段、升降序或空值排序：组合 `Comparator`**
   ```java
   ...
   ```
   使用 `comparing()`、`thenComparing()` 组合多字段规则；字段降序可传 `Comparator.reverseOrder()`，可能为空的对象或字段使用 `nullsFirst()`、`nullsLast()` 明确规则。

`List.sort()` 和 `Collections.sort()` 要求 List 支持 `set` 操作，`List.of()`、`Collections.unmodifiableList()` 等不可修改 List 会抛出 `UnsupportedOperationException`。对于有序 Stream，`sorted()` 是稳定排序；`List.sort()` 和 `Collections.sort()` 也会保持相等元素的原有相对顺序。

#### 常见追问

- `List.sort()` 和 `Collections.sort()` 有什么区别？
  两者都会原地修改 List，排序结果和稳定性语义一致。`List.sort()` 是 Java 8 在 `List` 接口上提供的默认方法，调用更贴近对象本身；`Collections.sort()` 是旧的工具类写法，维护旧代码时仍常见。新代码通常优先使用 `list.sort(comparator)`。
- 如何按对象多个字段排序？
  使用 `Comparator.comparing()` 配合 `thenComparing()` 组合排序规则。例如先按年龄升序、年龄相同再按 id 升序：`Comparator.comparing(User::getAge).thenComparing(User::getId)`。某个字段需要降序时，可使用 `Comparator.comparing(User::getAge, Comparator.reverseOrder())`。
- 原集合不想被修改怎么办？
  可以先复制再排序：`List<User> copy = new ArrayList<>(users); copy.sort(comparator);`。更直观的方式是 `users.stream().sorted(comparator).toList()`，它会生成新的 List，不修改原集合。
- List 中有 `null` 如何排序？
  自然排序和多数比较器遇到 `null` 通常会抛出 `NullPointerException`。可以显式指定空值位置，例如 `Comparator.nullsLast(Comparator.naturalOrder())`；按对象字段排序时，也要同时考虑对象本身和字段值是否可能为 `null`。
...
- 多线程场景能直接使用 `Vector` 吗？
  ...
  多线程读多写少场景，用 CopyOnWriteArrayList。
...
- 字符流底层是不是也依赖字节流？
  ...
  FileReader 本质上也可以理解为基于文件字节输入再解码成字符：JDK 8 的 FileReader 只能使用默认字符集；JDK 11+ 才支持在构造器中指定 Charset。JDK 8 需要指定编码时，应使用 `new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8)`。
...
BIO 常用于同步阻塞模型，一个连接通常对应一个线程，模型简单但高并发下线程开销大；
NIO 提供 Channel、Buffer 和 Selector。网络场景中，可将 SelectableChannel 配置为非阻塞并配合 Selector 实现同步多路复用，以少量线程管理大量连接；并非所有 NIO API 都是非阻塞的；
...
- Netty 和 Java NIO 有什么关系？
  NIO 提供非阻塞 IO 的基础能力，Netty 在 NIO 之上封装了线程模型、事件模型和编解码能力，更容易开发高性能、高并发的网络应用。

#### 关联文档

- [IO 与 NIO](/java-core/io-nio)
...
- `Queue` 和 `Deque` 有什么区别？
  Queue 通常按 FIFO 队列使用，从队尾插入元素、从队头取出元素，但接口本身不强制 FIFO；例如 PriorityQueue 按优先级出队。
...
- 阻塞队列有哪些常见实现？
  ...
  **PriorityBlockingQueue**：支持优先级的无界阻塞队列，元素需要具备自然排序能力，或在构造时提供 Comparator，不保证 FIFO。
...
- `Class.newInstance()` 为什么不推荐？
  它只能调用无参构造方法，并且受调用者访问权限限制；对构造方法内部抛出的异常处理也不清晰，可能直接透传异常并绕过编译期检查。`Class.newInstance()` 已在 Java 9 起废弃。
  更推荐使用 `clazz.getDeclaredConstructor(...).newInstance()`，可以明确选择构造方法，构造方法内部异常会包装为 `InvocationTargetException`。JDK 8 中可在权限允许时用 `setAccessible(true)` 访问非公开构造方法；JDK 9+ 还要求目标模块开放对应包，建议使用 `trySetAccessible()` 并处理失败情况。
- `clone()` 是深拷贝还是浅拷贝？
...
- [垃圾回收](/java-core/garbage-collection)

## 序列化与对象拷贝

### Serializable 接口有什么作用？

`Serializable` 是 Java 原生序列化的标记接口，声明该类可以参与默认序列化。默认序列化还要求对象图中可达的非 `static`、非 `transient` 字段同样可序列化，否则可能抛出 `NotSerializableException`；也可以通过自定义 `writeObject`、`readObject` 处理特殊字段。
它常用于对象持久化、网络传输、缓存和早期 RMI 场景。实际项目中更常见的是 JSON、Protobuf、Kryo 等序列化方案，因为 Java 原生序列化性能、兼容性和安全性都有局限。
...
- `transient` 字段会不会被序列化？
  默认 Java 序列化会忽略 `transient` 字段，反序列化后该字段是默认值。自定义 `writeObject()` 或 `Externalizable` 实现仍可以显式写入并恢复它的值。
- Java 原生序列化有什么安全风险？
...
- 为什么不推荐滥用 `clone()`？
  ...
  **实际开发里一般更倾向用拷贝构造器、静态工厂方法，或者明确的 DTO 转换工具，比如 BeanUtils、MapStruct。这样拷贝逻辑更清晰，也更容易维护。**
...
**单例模式**：通常保证同一个 `ClassLoader` 所定义的类只有一个实例，并提供全局访问点；跨类加载器、跨进程或分布式场景需要额外协调。
...
- 什么是单例模式懒汉式？
  懒汉式单例不会在类加载时立即创建实例，而是在第一次调用获取方法时才初始化，适合实例创建成本较高、未必一定会使用的场景。
  朴素的懒汉式在多线程下可能同时创建多个实例，因此生产中常用静态内部类 Holder 写法：
  ```java
  ...
  ```
  `Holder` 只有在首次调用 `getInstance()` 时才会初始化，Java 类初始化过程由 JVM 保证线程安全，因此能同时实现延迟加载和线程安全。
  也可以使用双重检查锁，但实例字段必须配合 `volatile`，避免指令重排导致其他线程拿到未完成初始化的对象。单例通常只保证同一 `ClassLoader` 下唯一；反射、序列化和克隆仍可能破坏单例，需要按场景额外防护。
- 简单工厂、工厂方法、抽象工厂有什么区别？
