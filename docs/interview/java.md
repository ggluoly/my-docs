...
- `String` 的 `+` 是不是运算符重载？
  ...
  类似"a" + "b"，编译器通常会转换为 StringBuilder 的 append() 调用；如果是编译期常量，可能直接折叠为一个字符串常量。
- Java 为什么保留方法重载？
  为了提升 API 的易用性和表达能力。
...
- 默认访问和 `protected` 有什么区别？
  ...
  **protected**：同包内任何类都能访问；不同包时，只有子类可以访问。
- 顶级类可以用哪些访问修饰符？
...
- `char[]` 一定安全吗？
  ...
  **char[] 不是“一定安全”，只是给了我们“尽早清除敏感数据”的能力。**
- 为什么日志里不能打印密码？
...
- 为什么重写 `equals()` 必须重写 `hashCode()`？
   Java 规定：如果两个对象 equals() 相等，那么它们的 hashCode() 必须相等。只重写 equals()，不重写 hashCode()，就可能出现两个逻辑上相等的对象，hashCode 却不一样。
