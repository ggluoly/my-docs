...
| 组件 | 定位 | 适用场景 |
| --- | --- | --- |
| `RocketMQ` | 业务消息、事务消息、延迟消息 | 订单、交易、电商业务 |
| `Kafka` | 高吞吐日志和流式数据 | 日志、埋点、实时计算 |
| `Pulsar` | 云原生消息与流式数据平台 | 多租户、消息与流统一、分层存储 |
| `RabbitMQ` | 传统 AMQP 消息队列 | 中小系统、通知、异步任务 |
...
```text
业务消息、事务消息、延迟消息：RocketMQ
日志、埋点、实时计算：Kafka
云原生多租户、消息与流统一：Pulsar
传统 AMQP、通知和轻量异步任务：RabbitMQ
```
